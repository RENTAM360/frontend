...
export interface BookingCustomer extends BookingUser {}

export interface BookingWithRole {
  isCustomer: boolean;
  booking: Booking;
}
...
export const equipmentApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    bookEquipment: builder.mutation<
      BookingResponse,
      { equipmentId: string; bookingData: BookingRequest }
    >({
    ...
    getEquipmentsByCategoryId: builder.query<
      { equipments: TransformedEquipment[]; totalCount: number },
      string
    >({
      query: (categoryId) =>
        `equipment?categoryId=${encodeURIComponent(categoryId)}`,
      providesTags: ["Equipment"],
      transformResponse: (response: EquipmentResponse) => {
        const transformedEquipments = response.data.equipments.map(
          (item: Equipment) => ({
    ...
    createCategory: builder.mutation<
      { status: number; message: string; data: Category },
      { name: string }
    >({
      query: (body) => ({
    ...
    createEquipmentReview: builder.mutation<
      ...
      { equipmentId: string; review: CreateReviewRequest }
    >({
    ...
    getEquipmentReviews: builder.query<
      { reviews: Review[]; totalCount: number },
      string
    >({
    ...
    bookmarkEquipment: builder.mutation<
      {
        status: number;
        message: string;
        data: { equipmentId: string; bookmarked: boolean };
      },
      string
    >({
      query: (equipmentId) => ({
    ...
    getAddressSuggestions: builder.query<
      { data: string[]; message: string; status: string },
      string
    >({
      query: (address) => ({
    ...
    getBookingById: builder.query<BookingWithRole, string>({
